(() => {
  ...
  var Router = class {
    ...
    handleRequest({ request, event }) {
      ...
      const catchHandler2 = route && route.catchHandler;
      if (responsePromise instanceof Promise && (this._catchHandler || catchHandler2)) {
        responsePromise = responsePromise.catch(async (err) => {
          if (catchHandler2) {
            ...
            try {
              return await catchHandler2.handle({ url, request, event, params });
            } catch (catchErr) {
  ...
  // ns-hugo-imp:C:\Users\martijn.vanden.boom\AppData\Local\hugo_cache\modules\filecache\modules\pkg\mod\github.com\razonyang\hugo-theme-bootstrap@v1.12.0\assets\service-worker\precache.ts
  var styleReg = new RegExp(
    /<link data-precache.*?rel="?stylesheet"?.*?href="?([^"\s]+)"?[^>]*>/g
  );
  var scriptReg = new RegExp(
    /<script data-precache.*?src="?([^"\s]+)"?[^>]*><\/script>/g
  );
  function precacheAssets(config2) {
    return fetch(config2.homepage).then((response) => {
      ...
      return self.caches.open(config2.stylesCacheName).then((cache) => cache.addAll(assets.styles)) && self.caches.open(config2.scriptsCacheName).then((cache) => cache.addAll(assets.scripts));
    }).catch((error) => {
      console.error("unable to fetch and precache styles", error);
    });
  }
  var precache_default = precacheAssets;

  // ns-hugo-imp:C:\Users\martijn.vanden.boom\AppData\Local\hugo_cache\modules\filecache\modules\pkg\mod\github.com\razonyang\hugo-theme-bootstrap@v1.12.0\assets\service-worker\handlers\catch.ts
  function catchHandler(config2) {
    const baseURL = new URL(config2.baseURL);
    return async (options) => {
      const cache = await self.caches.open(config2.fallbacksCacheName);
      let fallback = "/offline/";
      if (options.request.destination === "image") {
        fallback = config2.fallbacksImage;
      } else {
        const lang = getLanguageFromRequest(options.request, config2);
        if (lang !== "") {
  ...
  function getLanguageFromRequest(request, config2) {
    let url = request.url.toString();
    url = url.replace(config2.baseURL, "");
    const paths = url.split("/");
    if (paths.length > 0 && config2.langs.includes(paths[0])) {
      return paths[0];
    }
    return "";
  }
  var catch_default = catchHandler;
  ...
  var CacheableResponse = class {
    ...
    constructor(config2 = {}) {
      if (true) {
        if (!(config2.statuses || config2.headers)) {
          throw new WorkboxError2("statuses-or-headers-required", {
        ...
        if (config2.statuses) {
          finalAssertExports2.isArray(config2.statuses, {
            moduleName: "workbox-cacheable-response",
            className: "CacheableResponse",
            funcName: "constructor",
        ...
        if (config2.headers) {
          finalAssertExports2.isType(config2.headers, "object", {
            moduleName: "workbox-cacheable-response",
            className: "CacheableResponse",
            funcName: "constructor",
      ...
      this._statuses = config2.statuses;
      this._headers = config2.headers;
    }
    /**
    ...
    isResponseCacheable(response) {
      if (true) {
        finalAssertExports2.isInstance(response, Response, {
          moduleName: "workbox-cacheable-response",
          className: "CacheableResponse",
      ...
      if (true) {
        if (!cacheable) {
          logger2.groupCollapsed(`The request for '${getFriendlyURL2(response.url)}' returned a response that does not meet the criteria for being cached.`);
          logger2.groupCollapsed(`View cacheability criteria here.`);
          logger2.log(`Cacheable statuses: ` + JSON.stringify(this._statuses));
          logger2.log(`Cacheable headers: ` + JSON.stringify(this._headers, null, 2));
          logger2.groupEnd();
          const logFriendlyHeaders = {};
          response.headers.forEach((value, key) => {
            logFriendlyHeaders[key] = value;
          });
          logger2.groupCollapsed(`View response status and headers here.`);
          logger2.log(`Response status: ${response.status}`);
          logger2.log(`Response headers: ` + JSON.stringify(logFriendlyHeaders, null, 2));
          logger2.groupEnd();
          logger2.groupCollapsed(`View full response details here.`);
          logger2.log(response.headers);
  ...
  var CacheableResponsePlugin = class {
    ...
    constructor(config2) {
      ...
      this._cacheableResponse = new CacheableResponse(config2);
    }
  };
  ...
  var CacheExpiration = class {
     ...
     */
    constructor(cacheName, config2 = {}) {
      this._isRunning = false;
      this._rerunRequested = false;
      if (true) {
        finalAssertExports3.isType(cacheName, "string", {
          moduleName: "workbox-expiration",
          className: "CacheExpiration",
          funcName: "constructor",
          paramName: "cacheName"
        });
        if (!(config2.maxEntries || config2.maxAgeSeconds)) {
          throw new WorkboxError3("max-entries-or-age-required", {
            moduleName: "workbox-expiration",
            className: "CacheExpiration",
            funcName: "constructor"
          });
        ...
        if (config2.maxEntries) {
          finalAssertExports3.isType(config2.maxEntries, "number", {
            moduleName: "workbox-expiration",
            className: "CacheExpiration",
            funcName: "constructor",
        ...
        if (config2.maxAgeSeconds) {
          finalAssertExports3.isType(config2.maxAgeSeconds, "number", {
            moduleName: "workbox-expiration",
            className: "CacheExpiration",
            funcName: "constructor",
      ...
      this._maxEntries = config2.maxEntries;
      this._maxAgeSeconds = config2.maxAgeSeconds;
      this._matchOptions = config2.matchOptions;
      this._cacheName = cacheName;
    ...
    async expireEntries() {
      ...
      if (true) {
        if (urlsExpired.length > 0) {
          logger3.groupCollapsed(`Expired ${urlsExpired.length} ${urlsExpired.length === 1 ? "entry" : "entries"} and removed ${urlsExpired.length === 1 ? "it" : "them"} from the '${this._cacheName}' cache.`);
          logger3.log(`Expired the following ${urlsExpired.length === 1 ? "URL" : "URLs"}:`);
          urlsExpired.forEach((url) => logger3.log(`    ${url}`));
          logger3.groupEnd();
        } else {
          logger3.debug(`Cache expiration ran and found no entries to remove.`);
        }
      }
      this._isRunning = false;
    ...
    async updateTimestamp(url) {
      if (true) {
        finalAssertExports3.isType(url, "string", {
          moduleName: "workbox-expiration",
          className: "CacheExpiration",
    ...
    async isURLExpired(url) {
      if (!this._maxAgeSeconds) {
        if (true) {
          throw new WorkboxError3(`expired-test-without-max-age`, {
  ...
  function registerQuotaErrorCallback(callback) {
    if (true) {
      finalAssertExports3.isType(callback, "function", {
    ...
    if (true) {
      logger3.log("Registered a callback to respond to quota errors.", callback);
    }
  }
  ...
  var ExpirationPlugin = class {
      ...
      this.cachedResponseWillBeUsed = async ({ event, request, cacheName, cachedResponse }) => {
        ...
        if (event) {
          try {
            ...
            if (true) {
              if ("request" in event) {
                logger3.warn(`Unable to ensure service worker stays alive when updating cache entry for '${getFriendlyURL3(event.request.url)}'.`);
              }
            }
          }
        }
    ...
    _getCacheExpiration(cacheName) {
      if (cacheName === cacheNames.getRuntimeName()) {
        throw new WorkboxError3("expire-custom-caches-only");
      }
  ...
  async function executeQuotaErrorCallbacks() {
    if (true) {
      logger4.log(`About to run ${quotaErrorCallbacks2.size} callbacks to clean up caches.`);
    }
    for (const callback of quotaErrorCallbacks2) {
      await callback();
      if (true) {
        logger4.log(callback, "is complete.");
      }
    }
    if (true) {
      logger4.log("Finished running callbacks.");
    }
  }
  ...
  var StrategyHandler = class {
    ...
    constructor(strategy, options) {
      this._cacheKeys = {};
      if (true) {
        finalAssertExports4.isInstance(options.event, ExtendableEvent, {
    ...
    async fetch(input) {
      ...
      if (request.mode === "navigate" && event instanceof FetchEvent && event.preloadResponse) {
        ...
        if (possiblePreloadResponse) {
          if (true) {
            logger4.log(`Using a preloaded navigation response for '${getFriendlyURL4(request.url)}'`);
          }
      ...
      try {
        ...
        if (err instanceof Error) {
          throw new WorkboxError4("plugin-error-request-will-fetch", {
      ...
      try {
        ...
        if (true) {
          logger4.debug(`Network request for '${getFriendlyURL4(request.url)}' returned a response with status '${fetchResponse.status}'.`);
        }
        ...
        if (true) {
          logger4.log(`Network request for '${getFriendlyURL4(request.url)}' threw an error.`, error);
        }
        if (originalRequest) {
    ...
    async cacheMatch(key) {
      ...
      if (true) {
        if (cachedResponse) {
          logger4.debug(`Found a cached response in '${cacheName}'.`);
        } else {
          logger4.debug(`No cached response found in '${cacheName}'.`);
        }
      }
    ...
    async cachePut(key, response) {
      ...
      if (true) {
        if (effectiveRequest.method && effectiveRequest.method !== "GET") {
          throw new WorkboxError4("attempt-to-cache-non-get-request", {
            url: getFriendlyURL4(effectiveRequest.url),
            method: effectiveRequest.method
          });
        }
        const vary = response.headers.get("Vary");
        if (vary) {
          logger4.debug(`The response for ${getFriendlyURL4(effectiveRequest.url)} has a 'Vary: ${vary}' header. Consider setting the {ignoreVary: true} option on your strategy to ensure cache matching and deletion works as expected.`);
        }
      }
      if (!response) {
        if (true) {
          logger4.error(`Cannot cache non-existent response for '${getFriendlyURL4(effectiveRequest.url)}'.`);
        }
        throw new WorkboxError4("cache-put-with-no-response", {
          url: getFriendlyURL4(effectiveRequest.url)
        });
      }
      ...
      if (!responseToCache) {
        if (true) {
          logger4.debug(`Response '${getFriendlyURL4(effectiveRequest.url)}' will not be cached.`, responseToCache);
        }
        return false;
      }
      const { cacheName, matchOptions } = this._strategy;
      ...
      if (true) {
        logger4.debug(`Updating the '${cacheName}' cache with a new Response for ${getFriendlyURL4(effectiveRequest.url)}.`);
      }
      try {
          ...
          if (responseToCache) {
            if (responseToCache.status !== 200) {
              if (responseToCache.status === 0) {
                logger4.warn(`The response for '${this.request.url}' is an opaque response. The caching strategy that you're using will not cache opaque responses by default.`);
              } else {
                logger4.debug(`The response for '${this.request.url}' returned a status code of '${response.status}' and won't be cached as a result.`);
              }
            }
          }
        }
    ...
    async _getResponse(handler, request, event) {
      ...
      try {
        ...
        if (!response || response.type === "error") {
          throw new WorkboxError4("no-response", { url: request.url });
        }
      } catch (error) {
        if (error instanceof Error) {
        ...
        if (!response) {
          ...
          logger4.log(`While responding to '${getFriendlyURL4(request.url)}', an ${error instanceof Error ? error.toString() : ""} error occurred. Using a fallback response provided by a handlerDidError plugin.`);
        }
      }
  ...
  var messages5 = {
    strategyStart: (strategyName, request) => `Using ${strategyName} to respond to '${getFriendlyURL4(request.url)}'`,
    printFinalResponse: (response) => {
      if (response) {
        logger4.groupCollapsed(`View the final response here.`);
        logger4.log(response || "[No response returned]");
        logger4.groupEnd();
      }
    }
  };
  ...
  var CacheFirst = class extends Strategy {
    ...
    async _handle(request, handler) {
      const logs = [];
      if (true) {
        finalAssertExports4.isInstance(request, Request, {
          moduleName: "workbox-strategies",
      ...
      if (true) {
        logger4.groupCollapsed(messages5.strategyStart(this.constructor.name, request));
        for (const log of logs) {
          logger4.log(log);
        }
        messages5.printFinalResponse(response);
        logger4.groupEnd();
      }
      if (!response) {
        throw new WorkboxError4("no-response", { url: request.url, error });
      }
      return response;
    }
  };
    ...
    constructor(options = {}) {
      ...
      if (true) {
        if (this._networkTimeoutSeconds) {
          finalAssertExports4.isType(this._networkTimeoutSeconds, "number", {
            moduleName: "workbox-strategies",
            className: this.constructor.name,
            funcName: "constructor",
  ...
  // ns-hugo-imp:C:\Users\martijn.vanden.boom\AppData\Local\hugo_cache\modules\filecache\modules\pkg\mod\github.com\razonyang\hugo-theme-bootstrap@v1.12.0\assets\service-worker\routes\font.ts
  function fontRoute(config2) {
    return new Route(
      ({ request }) => {
        return request.destination === "font";
      },
      new CacheFirst({
        cacheName: config2.fontsCacheName,
        plugins: [
  ...
  var font_default = fontRoute;

  // ns-hugo-imp:C:\Users\martijn.vanden.boom\AppData\Local\hugo_cache\modules\filecache\modules\pkg\mod\github.com\razonyang\hugo-theme-bootstrap@v1.12.0\assets\service-worker\routes\image.ts
  function imageRoute(config2) {
    return new Route(
      ({ request }) => {
        return request.destination === "image";
      },
      new CacheFirst({
        cacheName: config2.imagesCacheName,
        plugins: [
          new CacheableResponsePlugin({
            statuses: [200]
          }),
  ...
  var image_default = imageRoute;

  // ns-hugo-imp:C:\Users\martijn.vanden.boom\AppData\Local\hugo_cache\modules\filecache\modules\pkg\mod\github.com\razonyang\hugo-theme-bootstrap@v1.12.0\assets\service-worker\routes\page.ts
  function networkFirst(config2) {
    return new NetworkFirst({
      cacheName: config2.pagesCacheName,
      plugins: [
        new CacheableResponsePlugin({
          statuses: [200]
        })
  ...
  function pageRoute(config2) {
    return new Route(({ request }) => {
      return request.mode === "navigate";
    }, networkFirst(config2));
  }
  var page_default = pageRoute;

  // ns-hugo-imp:C:\Users\martijn.vanden.boom\AppData\Local\hugo_cache\modules\filecache\modules\pkg\mod\github.com\razonyang\hugo-theme-bootstrap@v1.12.0\assets\service-worker\routes\script.ts
  function scriptRoute(config2) {
    return new Route(
      ({ request, sameOrigin }) => {
        return sameOrigin && request.destination === "script";
      },
      new CacheFirst({
        cacheName: config2.scriptsCacheName,
        plugins: [
          new CacheableResponsePlugin({
            statuses: [200]
          }),
  ...
  var script_default = scriptRoute;

  // ns-hugo-imp:C:\Users\martijn.vanden.boom\AppData\Local\hugo_cache\modules\filecache\modules\pkg\mod\github.com\razonyang\hugo-theme-bootstrap@v1.12.0\assets\service-worker\routes\style.ts
  function styleRoute(config2) {
    return new Route(
      ({ request }) => {
        return request.destination === "style";
      },
      new CacheFirst({
        cacheName: config2.stylesCacheName,
        plugins: [
          new CacheableResponsePlugin({
            statuses: [200]
          }),
  ...
  var style_default = styleRoute;

  // <stdin>
  var precache = JSON.parse("{}");
  var config = {
    env: "development",
    baseURL: "http://localhost:1313/",
    homepage: "/en/",
    langs: JSON.parse('["en","nl","fr","zh-hant","zh-hans","ar","ja"]'),
    fallbacks: JSON.parse('["/en/offline/","http://localhost:1313/en/manifest.json","http://localhost:1313/nl/manifest.json","http://localhost:1313/fr/manifest.json","/zh-hant/offline/","http://localhost:1313/zh-hant/manifest.json","/zh-hans/offline/","http://localhost:1313/zh-hans/manifest.json","http://localhost:1313/ar/manifest.json","/ja/offline/","http://localhost:1313/ja/manifest.json","/images/offline.png"]'),
    fallbacksCacheName: "fallbacks",
    fallbacksImage: "images/offline.png",
  ...
  registerRoute(font_default(config));
  registerRoute(image_default(config));
  registerRoute(page_default(config));
  registerRoute(script_default(config));
  registerRoute(style_default(config));
  self.addEventListener("install", (event) => {
    if (config.fallbacks) {
      event.waitUntil(
        caches.open(config.fallbacksCacheName).then((cache) => cache.addAll(config.fallbacks)).catch((error) => {
          console.log("unable to cache fallbacks:", error);
        })
      );
    }
    event.waitUntil(precache_default(config));
    if (config.fonts) {
      event.waitUntil(
        caches.open(config.fontsCacheName).then((cache) => cache.addAll(config.fonts)).catch((error) => {
    ...
    if (config.images) {
      event.waitUntil(
        caches.open(config.imagesCacheName).then((cache) => cache.addAll(config.images)).catch((error) => {
    ...
    if (config.pages) {
      event.waitUntil(
        caches.open(config.pagesCacheName).then((cache) => cache.addAll(config.pages)).catch((error) => {
    ...
    if (config.scripts) {
      event.waitUntil(
        caches.open(config.scriptsCacheName).then((cache) => cache.addAll(config.scripts)).catch((error) => {
    ...
    if (config.styles) {
      event.waitUntil(
        caches.open(config.stylesCacheName).then((cache) => cache.addAll(config.styles)).catch((error) => {
          console.log("unable to cache styles:", error);
        })
      );
    }
  });
  setCatchHandler(catch_default(config));
})();
